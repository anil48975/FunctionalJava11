...
/**
 * This class is responsible for expression evaluation/calculation, provided as input string.
 *
 * This class has 2 public overloaded method evaluateExpression(), one take input and provides the result,
 *  while another allows to provide a validator and expression parser to configure the class as per requirement.
 *
 *  If a client program needs to test in multithreaded environment, it can use this classes' [task] BiFunction field
 *  to submit the task to an ExecuterService.
 *
 *  The class is thread safe as there is no shared instance variable.
 *
 *  This class is self contained, all helper classes are declared as inner classes.
 */
public class EvalExpression {
//_________________________________________________________________________________________________________________________
// Public Fields declarations
public static BiFunction<String, EvalExpression, Callable<Integer>> task = (input, evalExpression) -> {
    return () -> {
        try {
            return evalExpression.evaluateExpression(input);
        } catch(Exception e) {
...
//________________________________________________________________________________________________________________________
    public static void main(String[] args) {
        ...
        try {
            System.out.println("Expected Result: 22 :: Actual result: " + executorService.submit(task.apply("7+8/2-1+3*4", evalExpression)).get());
            System.out.println("Expected Result: 26 :: Actual result: " + executorService.submit(task.apply("7+5*2/2+14", evalExpression)).get());
            System.out.println("Expected Result: 17 :: Actual result: " + executorService.submit(task.apply("7+5*2/2+(14-4)/2", evalExpression)).get());
            System.out.println("Expected Result: 17 :: Actual result: " + executorService.submit(task.apply("(7+5*2/2+(14-4)/2)", evalExpression)).get());
            System.out.println("Expected Result: 32 :: Actual result: " + executorService.submit(task.apply("(7+5*(2/2+(14-5))/2)", evalExpression)).get());
            System.out.println("Expected Result: -1 :: Actual result: " + executorService.submit(task.apply("(7+5*(2/2+(14-5))/ 2)", evalExpression)).get());
    ...
    /**
     * This method takes a input string containing an expression and returns the expression value.
     *
     * @param input string containing an expression
     *
     * @return calculated value of provided expression or -1 if any exception occurs and logs the exception in console.
     */
     ...
     *
     * @param input string containing an expression
     * @param parseExpression this function if provided by the client program, will be used by this method in place of class's own parsing function
     * @param validator this function/consumer if provided by the client program, will be used by this method in place of class's own validator function/consumer
     *
     * @return calculated value of provided expression or -1 if any exception occurs and logs the exception in console.
     */
...
// private methods ______________________________________________________________________________________________________________
    ...
    private Stack<String> solveBrackets(Stack<String> expressionValues) {
        ...
        return reverseStack.apply(newExpressionValues);
    }
    ...
    private int evaluateExpression(Stack<String> expressionValues) {
        ...
        while (!expressionValues.isEmpty()) {
            ...
            if (isNumber.apply(expressionValue)) {
    ...
    private static final Consumer<String> validator = input -> {
        ...
        for (int i = 0; i < inputChars.length; i++) {
            ...
            if (!(isNumber.apply(singleInput) || Brackets.OPENING.toString().equals(singleInput) || Brackets.CLOSING.toString().equals(singleInput)
                ...
                throw new RuntimeException("\""+singleInput+"\"" + " is not a valid input, for input string " + input +" at index " + i + ".");
            }
        }
    };
}